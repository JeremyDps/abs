...
class DBClass
{
    ...
    function selectEtuWithoutGroup() {
        ...
        $query = $this->getPDO()->query("SELECT idEtu, prenom, nom, formation, nbr_absence FROM `etudiant` 
                                                  WHERE groupe_id < 1 or groupe_id > (select COUNT(*) from groupe) + (select count(*) from groupe_tp)");
